...
namespace HotPathAllocationAnalyzer.Configuration
{
    public class ConfigurationReader
    {
        ...
        public async Task<IEnumerable<string>> GenerateWhitelistAsync(CancellationToken cancellationToken)
        {
            ...
            foreach (var document in _configurationProject.Documents)
            {
                ...
                {
                    var configurationClasses = GetConfigurationClasses(await syntaxTree.GetRootAsync(cancellationToken), semanticModel);
                    whiteListSymbols.UnionWith(configurationClasses.SelectMany(x => GenerateWhitelistSymbols(x, semanticModel, cancellationToken)));
                }
            }
        ...
        private static IEnumerable<string> GenerateWhitelistSymbols(ClassDeclarationSyntax classDecl, SemanticModel semanticModel, CancellationToken cancellationToken)
            => classDecl.Members.OfType<MethodDeclarationSyntax>().SelectMany(m => GenerateWhitelistSymbol(m, semanticModel, cancellationToken));

        private static IEnumerable<string> GenerateWhitelistSymbol(MethodDeclarationSyntax methodDecl, SemanticModel semanticModel, CancellationToken cancellationToken)
        {
            var statements = methodDecl.Body?.Statements ?? new SyntaxList<StatementSyntax>();

            foreach (var p in GenerateWhitelistSymbolsFromExpressions(semanticModel, statements, cancellationToken))
                yield return p;
            ...
            foreach (var p in GenerateWhitelistSymbolsFromInvocations(semanticModel, statements, cancellationToken))
                yield return p;
        }

        private static IEnumerable<string> GenerateWhitelistSymbolsFromExpressions(SemanticModel semanticModel, SyntaxList<StatementSyntax> statements, CancellationToken cancellationToken)
        {
            ...
            {
                var symbol = semanticModel.GetSymbolInfo(invocationExpr, cancellationToken).Symbol;
                ...
                switch (symbol?.Name)
                {
                    ...
                    {
                        var childSymbol = semanticModel.GetSymbolInfo(lambdaExpr.Body, cancellationToken).Symbol;
                    ...
                    {
                        var childSymbol = semanticModel.GetSymbolInfo(typeOfExpressionSyntax.Type, cancellationToken).Symbol;
                        if (childSymbol?.ToString() is { } p)
                            yield return p;
                        break;
                    }
                }
        ...
        private static IEnumerable<string> GenerateWhitelistSymbolsFromInvocations(SemanticModel semanticModel, SyntaxList<StatementSyntax> statements, CancellationToken cancellationToken)
        {
            var invocationsExpr = statements.OfType<ExpressionStatementSyntax>()
                                            .Select(x => x.Expression)
                                            .OfType<InvocationExpressionSyntax>();
            ...
            foreach (var invocationExpr in invocationsExpr)
            {
                var childSymbol = semanticModel.GetSymbolInfo(invocationExpr, cancellationToken).Symbol;
                foreach (var p in SerializeSymbol(childSymbol))
                    yield return p;
            }
        }
